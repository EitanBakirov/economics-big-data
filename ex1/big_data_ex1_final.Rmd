...
from: https://www.airbnb.com/help/article/828

```{r}
# Filter out rows with missing values in host_response_time or host_is_superhost
...
# Visualization
ggplot(response_time_summary, aes(x = host_response_time, y = count, fill = host_is_superhost)) +
  geom_bar(stat = "identity", position = "dodge") +
  geom_text(aes(label = count), position = position_dodge(width = 0.9), vjust = -0.5, size = 3.5) +
  labs(title = "Host Response Time by Superhost Status",
       x = "Host Response Time",
       y = "Count") +
  theme_minimal() +
  theme(plot.title = element_text(hjust = 0.5)) +
  scale_fill_brewer(palette = "Set3", name = "Superhost Status")
...
# Visualization
ggplot(response_time_summary, aes(x = host_response_time, y = percentage, fill = host_is_superhost)) +
  geom_bar(stat = "identity", position = "dodge") +
  geom_text(aes(label = paste0(round(percentage, 1), "%")), position = position_dodge(width = 0.9), vjust = -0.5, size = 3.5) +
  labs(title = "Host Response Time by Superhost Status",
       x = "Host Response Time",
       y = "Percentage") +
  theme_minimal() +
...
```{r}
# Filter out rows with missing values in review_scores_rating or host_is_superhost
filtered_data <- listings_data %>%
  filter(!is.na(review_scores_rating) & !is.na(host_is_superhost))

# Create a new column for score groups
filtered_data <- filtered_data %>%
  mutate(score_group = case_when(
    review_scores_rating >= 90 ~ "High",
    review_scores_rating >= 50 & review_scores_rating < 90 ~ "Medium",
    review_scores_rating < 50 ~ "Low"
  ))

# Calculate summary statistics
score_group_summary <- filtered_data %>%
  group_by(host_is_superhost, score_group) %>%
  summarise(count = n()) %>%
  mutate(total = sum(count), percentage = (count / total) * 100)

print(score_group_summary)

# Visualization
ggplot(score_group_summary, aes(x = score_group, y = percentage, fill = host_is_superhost)) +
  geom_bar(stat = "identity", position = "dodge") +
  geom_text(aes(label = paste0(round(percentage, 1), "%")), position = position_dodge(width = 0.9), vjust = -0.5, size = 3.5) +
  labs(title = "Percentage of Superhosts and Non-Superhosts by Review Score Group",
       x = "Review Score Group",
       y = "Percentage") +
  theme_minimal() +
  theme(plot.title = element_text(hjust = 0.5)) +
  scale_fill_brewer(palette = "Set3", name = "Superhost Status")
...
# Visualization
ggplot(filtered_data, aes(x = host_is_superhost, y = review_scores_rating, fill = host_is_superhost)) +
  geom_boxplot() +
  labs(title = "Review Scores by Superhost Status",
       x = "Superhost Status",
...
# Filter out rows with missing values in review_scores_location or neighbourhood
filtered_data <- listings_data %>%
  filter(!is.na(review_scores_location) & !is.na(neighbourhood))

# Calculate average review score and percentage of properties in each neighborhood
neighbourhood_summary <- filtered_data %>%
  ...
  mutate(percentage = (count / sum(count)) * 100,
         neighborhood_label = paste0(neighbourhood, " (", round(percentage, 1), "%)"))

# Visualization
ggplot(neighbourhood_summary, aes(x = reorder(neighbourhood, -avg_review_score), y = avg_review_score, fill = neighborhood_label)) +
  geom_bar(stat = "identity") +
  geom_text(aes(label = round(avg_review_score, 1)), vjust = -0.5, size = 3.5) +
  labs(title = "Average Review Scores by Neighborhood with Property Percentage",
       x = "Neighborhood",
       y = "Average Review Score (Location)") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1),
        plot.title = element_text(hjust = 0.5)) +
  scale_fill_discrete(name = "The percentage of listings in each neighborhood from the total listings") +
  coord_cartesian(clip = "off")

```
רוב השכונות מקבלות ציון שביעות רבון ממוצע מהשכונה ממש גבוה ולא יורד מ7.6
...
```{r, echo=TRUE, fig.width=14, fig.height=10}
# Filter out rows with missing values in review_scores_location, price_dollars, or neighbourhood
filtered_data_neighbourhood <- listings_data %>%
  filter(!is.na(review_scores_location) & !is.na(price_dollars) & !is.na(neighbourhood))

# Create a faceted scatter plot by neighbourhood
ggplot(filtered_data_neighbourhood, aes(x = review_scores_location, y = price_dollars)) +
  geom_point(alpha = 0.6, size = 3, color = "blue") +
  labs(title = "Relationship Between Review Scores and Price by Neighbourhood",
       x = "Review Score (Location)",
       y = "Price (in dollars)") +
  theme_minimal() +
  theme(plot.title = element_text(hjust = 0.5)) +
...
# Plot the correlations with text annotations
ggplot(correlations, aes(x = reorder(Aspect, Correlation), y = Correlation, fill = Aspect)) +
  geom_bar(stat = "identity") +
  geom_text(aes(label = round(Correlation, 2)), vjust = -0.3, hjust = 0.5, color = "black", size = 3) +
  coord_flip() +
...
```{r, echo=TRUE, fig.width=14, fig.height=8}
# Filter out rows with missing values in review_scores_rating, price_dollars, or beds
filtered_data <- listings_data %>%
  filter(!is.na(review_scores_rating) & !is.na(price_dollars) & !is.na(beds))

# Create a faceted scatter plot
ggplot(filtered_data, aes(x = review_scores_rating, y = price_dollars)) +
  geom_point(alpha = 0.6, size = 3, color = "blue") +
  labs(title = "Relationship Between Average Scores and Price by Number of Beds",
       x = "Average Review Score (Rating)",
       y = "Price (in dollars)") +
  theme_minimal() +
  theme(plot.title = element_text(hjust = 0.5)) +
...
# Filter out rows with missing review_scores_location or price_dollars
neighborhood_data <- listings_data %>%
  filter(!is.na(review_scores_location) & !is.na(price_dollars)) %>%
  group_by(neighbourhood) %>%
  summarise(
    avg_review_score = mean(review_scores_location, na.rm = TRUE),
...
# Plot: Average Review Scores for Location by Neighbourhood with Average Price
ggplot(neighborhood_data, aes(x = reorder(neighbourhood, -avg_review_score), y = avg_review_score, size = avg_price, color = num_listings)) +
  geom_point(alpha = 0.6) +
  scale_size_area(max_size = 20) +
  scale_color_gradient(low = "lightblue", high = "blue") +
  labs(title = "Average Review Scores for Location by Neighbourhood with Average Price",
       x = "Neighbourhood",
       y = "Average Review Score (Location)",
       size = "Average Price (in dollars)",
...
6.
```{r}
# Prepare the data by filtering out NA values for the necessary columns
clean_data <- listings_data %>%
  filter(!is.na(review_scores_value) & !is.na(price_dollars))

# Create a scatter plot of price vs. review score value
ggplot(clean_data, aes(x = review_scores_value, y = price_dollars)) +
  geom_point(alpha = 0.4, color = "blue") +  # Semi-transparent points to handle overplotting
  labs(title = "Relationship Between Review Score Value and Price",
       x = "Review Score Value",
       y = "Price in Dollars") +
  theme_minimal() +
...
```{r}
# Filter out rows with missing values in review_scores_value or price_dollars
filtered_data <- listings_data %>%
  filter(!is.na(review_scores_value) & !is.na(price_dollars))

# Calculate the average price for each review score value
average_price_per_score <- filtered_data %>%
  group_by(review_scores_value) %>%
  summarise(avg_price_dollars = mean(price_dollars, na.rm = TRUE))

# Create a box plot
ggplot(filtered_data, aes(x = factor(review_scores_value), y = price_dollars)) +
  geom_boxplot(fill = "skyblue", color = "black") +
  labs(title = "Box Plot of Price by Review Score Value",
       x = "Review Score Value",
       y = "Price in Dollars") +
  theme_minimal() +
  theme(plot.title = element_text(hjust = 0.5))
```
גרף זה מראה שציוני ביקורת גבוהים יותר מציעים מגוון רחב יותר של מחירים, מה שמצביע על כך שערך ביקורת התאמה המחיר לתמורה אינו קשור רק למחירים נמוכים יותר. נכסים עם ציוני ערך-תמורה גבוהים (8-10) נפוצים בטווחי מחירים בינוניים עד גבוהים יותר, מה שמצביע על כך שנכסים יקרים יותר כנראה באמת משקיעים בלהחזיר תמורה לתשלום היקר של הלקוח, אף על פי שיש גם נכסים עם מחירים נמוכים שנותנים תמורה למחיר, זה פשוט יותר נפוץ שעבור נכסים זולים יותר הלקוחות יהיו פחות מרוצים מהתמורה שהם מקבלים.

ראשית, נכסים עם ציוני ביקורת גבוהים יותר (8-10) נוטים לקבל מגוון רחב יותר של מחירים, מה שמצביע על שונות רבה יותר במחיר עבור נכסים בעלי דירוג גבוה יותר של התמורה למחיר. שנית, ציוני ביקורת נמוכים יותר (2-6) קשורים בדרך כלל למחירים נמוכים יותר ומראה פחות שונות במחירים. לבסוף, המחירים הגבוהים ביותר נצפים בקטגוריות ציון הביקורות הגבוהות ביותר, מה שמצביע על כך שנכסים עם דירוג גבוה יותר יכולים לדרוש מחירים גבוהים יותר.

כדי למקסם את ההכנסה, על המארחים לשאוף לשפר את התמורה שהם נותנים בהתאם למחיר שהם מתקצבים את הנכס לילה,  הם יכולים לעשות זאת באמצעות שירות ושירותים טובים 
יותר, מכיוון שציונים גבוהים יותר קשורים ליכולת לגבות מחירים גבוהים יותר (כפי שרואים בגרף).

בנוסף, השונות ברישומים בעלי ניקוד גבוה מרמזת על פוטנציאל לאסטרטגיות אופטימיזציה של מחירים, שבהן המארחים יכולים להתנסות עם שכבות תמחור שונות כדי למצוא את האיזון האופטימלי בין תפוסה להכנסה.


```{r, echo=TRUE, fig.width=14, fig.height=8}
# Calculate the average price and count for each review score value
avg_price_by_score <- listings_data %>%
  filter(!is.na(review_scores_value) & !is.na(price_dollars)) %>%
  group_by(review_scores_value) %>%
  summarise(avg_price = mean(price_dollars), count = n())

# Plot the average price for each review score value with labels
ggplot(avg_price_by_score, aes(x = as.factor(review_scores_value), y = avg_price, fill = as.factor(review_scores_value))) +
  geom_bar(stat = "identity") +
  geom_text(aes(label = paste("AVG Price=", round(avg_price, 1))), vjust = -0.5, color = "black", size = 4) +
  geom_text(aes(y = 0, label = paste("N=", count)), vjust = 1.5, color = "red", size = 4) +
  labs(title = "Average Price by Review Score Value",
       x = "Review Score Value",
...
```{r}
# Ensure listings_data has a date column based on host_since
listings_data <- listings_data %>%
  mutate(host_since_month = format(as.Date(host_since), "%Y-%m"))

# Create a data frame with all months from the first to the last month in listings_data
all_months <- data.frame(host_since_month = seq(from = floor_date(min(as.Date(listings_data$host_since)), "month"),
                                                to = floor_date(max(as.Date(listings_data$host_since)), "month"),
                                                by = "month"))
all_months$host_since_month <- format(all_months$host_since_month, "%Y-%m")

# Count new listings per month
new_listings_per_month <- listings_data %>%
  group_by(host_since_month) %>%
  summarise(new_listings = n())

# Merge all months with new listings data
monthly_data <- all_months %>%
  left_join(new_listings_per_month, by = "host_since_month") %>%
  mutate(new_listings = ifelse(is.na(new_listings), 0, new_listings))

# Calculate the difference between the number of new properties that joined last month and how many joined this month
monthly_data <- monthly_data %>%
  mutate(delta_new_listings = new_listings - lag(new_listings, default = 0))

# Calculate the cumulative amount of new listings
monthly_data <- monthly_data %>%
  mutate(cumsum_new_listings = cumsum(new_listings)) 
...
```{r echo=TRUE, fig.width=18, fig.height=12}
# Plotting delta_new_listings
ggplot(monthly_data, aes(x = as.Date(paste0(host_since_month, "-01")), y = delta_new_listings)) +
  geom_line(color = "blue", size = 1) +
  geom_point(color = "blue", size = 1) +
  labs(title = "Monthly Change in New Listings",
       x = "Month",
       y = "Delta New Listings") +
  scale_x_date(date_labels = "%b %Y", date_breaks = "1 month") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
```
...
```{r echo=TRUE, fig.width=18, fig.height=12}
# Plotting cumsum_new_listings
ggplot(monthly_data, aes(x = as.Date(paste0(host_since_month, "-01")), y = cumsum_new_listings)) +
  geom_line(color = "green", size = 1) +
  geom_point(color = "green", size = 1) +
  labs(title = "Cumulative Sum of New Listings",
       x = "Month",
       y = "Cumulative New Listings") +
  scale_x_date(date_labels = "%b %Y", date_breaks = "1 month") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
```

קשה להבין מה קורה כל חודש אז נסתכל על השנים!!
...
# Ensure listings_data has a date column based on host_since
listings_data <- listings_data %>%
  mutate(host_since_year = year(as.Date(host_since)))

# Count new listings per year
new_listings_per_year <- listings_data %>%
...
```
חסר לי בנתונים את הלילות שהוצעו והיו בהם בוקינג באותם השנים, אז אין לי אפשרות לראות שההיצע ענה על הביקוש.

אני מראה את הצמיחה בנכסים של איירביאנדבי - גידול היצע הנכסים 
נותני השירות בפלטפורמה

אנחנו לא יודעים כמה לילות הפלטפורמה הציעה באותם השנים- אלו נתונים שחסרים, יכול להיות שיש מלא נכסים שנרשמו ולא נתנו הרבה לילות



```{r}
# Plot the new listings per year with labels on top of each bar
ggplot(new_listings_per_year, aes(x = host_since_year, y = new_listings)) +
  geom_bar(stat = "identity", fill = "skyblue") +
  geom_text(aes(label = new_listings), vjust = -0.3, color = "black", size = 3.5) +
  labs(title = "New Listings per Year",
       x = "Year",
       y = "New Listings") +
  scale_x_discrete(limits = unique(new_listings_per_year$host_since_year)) +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
```
נראה שבכל שנה יש מגמת עלייה בהיצע הנכסים שמבוקשים בפלטפורמה, נזכור ששנת 2008 מתחילה מנובמבר ושנת 2016 מסתיימת בספטמבר כך שצריך לקחת את הנתונים שם בערבון מוגבל

מדובר על צמיחה שנתית של הנכסים המוצעים בפלטפורמה עבור 2009 עד 2015:
רואים שיש מגמת עליה בכמות הנכסים המוצעים בכל שנה, כלומר מידי שנים יש עוד מארחים שנכנסים לפלטפורמה.


חסר:
האם העליה היא עלייה איכותית- כלומר האם הליסטינג שנרשמו הם "אקטיבים" , כלומר האם הם סתם נרשמו לאתר או באמת מציעים את הנכס שלהם אבל יש פה קצת מידע שחסר כי אין נתונים על ליסטינג אקטיביים.

אבללללללל
אולי ננסה להסתכל על ליסטינג אקטיבי
נגיד שליסטינג הוא אקטיבי אם בחודש יש לו לפחות בוקינג 1 בחודש.

Determine Active Listings: A listing can be considered active if it has at least one booking (available_category = 1) in a given month.

Group by Month: Group the data by month and count the number of unique active listings.

```{r}
# Ensure the merged_data has a date column
...
# Calculate the number of unique active listings per month
active_listings_per_month <- merged_data %>%
  filter(available_category == 1) %>%
  group_by(month_year) %>%
  summarise(active_listings = n_distinct(listing_id))

active_listings_per_month
```
```{r}
# Plot the number of active listings per month
ggplot(active_listings_per_month, aes(x = month_year, y = active_listings)) +
  geom_line(color = "blue", size = 1) +
  geom_point(color = "blue", size = 2) +
  labs(title = "Active Listings per Month",
       x = "Month-Year",
  ...
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
```
כאן אנחנו רואים שיש ירידה בכמות הליסטינג האקטיבים במהלך חודשי השנה בערך מאמצע נוב 2016
זה יכול להיות קצת בעייתי כי מדבור על טווח זמנים קצר - שנה בלבד כאשר כל חודש מושפע מלאאא גורמים סביבתיים

The graph shows the number of active listings per month from September 2016 to September 2017. The y-axis represents the number of active listings, and the x-axis shows the months within the specified time frame.
...
# Merge the listing counts with the neighborhood_revpan_data
neighborhood_revpan_data <- neighborhood_revpan_data %>%
  left_join(listings_count, by = "neighbourhood")

# Create label data with the listing counts
label_data <- neighborhood_revpan_data %>%
  group_by(neighbourhood) %>%
  summarise(year_month = last(year_month), RevPAN = last(RevPAN), label = paste(neighbourhood, "(", listing_count, ")", sep = ""))

# Plotting Monthly RevPAN for each neighborhood
ggplot(neighborhood_revpan_data, aes(x = year_month, y = RevPAN, color = neighbourhood, group = neighbourhood)) +
  geom_line(size = 1) +
  geom_point(size = 2) +
  geom_text(data = label_data, aes(label = label), hjust = 0, vjust = -0.5, size = 3, check_overlap = TRUE) +
  labs(title = "Monthly RevPAN by Neighborhood",
       x = "Month",
       y = "RevPAN (Revenue per Available Night)") +
  theme_minimal() +
