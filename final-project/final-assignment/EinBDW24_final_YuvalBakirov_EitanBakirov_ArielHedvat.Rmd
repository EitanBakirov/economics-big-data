...
We decided to remove the features that are weather related because we realized that
this data is about the weather around the Empire State Building, which does not represent 
well enough the whole data of 2000m radius and above.
Therefore we are removing them in order to get better predictions.

```{r}
features_to_remove <-  c("feels_like", "is_raining_last_hour")
train_filtered_2000m_data <- remove_features(train_filtered_2000m_data, features_to_remove)
```

```{r}
# Convert all integer columns to numeric in a data frame or data.table
convert_to_numeric <- function(data) {
  # Check if the data is a data.table
  if ("data.table" %in% class(data)) {
    # If data is a data.table, use data.table syntax
    int_cols <- names(data)[sapply(data, is.integer)]
    data[, (int_cols) := lapply(.SD, as.numeric), .SDcols = int_cols]
  } else {
    # If data is a data.frame, use data.frame syntax
    int_cols <- names(data)[sapply(data, is.integer)]
...
```{r}
train_filtered_2000m_data <- convert_to_numeric(train_filtered_2000m_data)
```
...
```{r}
plot_validation_mse_single <- function(results, models, data_name) {
  
  # Extract validation MSE from the results
  validation_mse_before <- sapply(models, function(model) results[[model]]$validation_mse)
  
  # Combine into a data frame for plotting
  mse_data <- data.frame(
    Model = models,
    Validation_MSE = validation_mse_before
  )
  
  # Plotting
  ggplot(mse_data, aes(x = Model, y = Validation_MSE, fill = Model)) +
    geom_bar(stat = "identity", position = "dodge") +
    geom_text(aes(label = round(Validation_MSE, 2)), 
              position = position_dodge(width = 0.9), 
              vjust = -0.3, 
              size = 3.5) +
    labs(
      title = paste("Validation MSE for Each Model -", data_name),
      x = "Model",
      y = "Validation Mean Squared Error (MSE)"
    ) +
    theme_minimal() +
...
```{r}
# Call the function with your results objects for before and after feature selection
plot_validation_mse_single(results = results_2000, models = models, data_name = "2000m")
```
...
```{r}
save_predictions_to_csv(test_1000_data_with_predictions, "EinBDW24_final_pred_c1_YuvalBakirov_EitanBakirov_ArielHedvat.csv")
```


For 2000:

As we explained before, the best model we found was Catboost without the features: 
- "feels_like"
- "is_raining_last_hour"

which got us Average MSE Validation using CV of 626.53

Thus, we should remove these features 
(Final 2000 data - train_filtered_2000m_data)
(Final 2000 model - results_2000$catboost$model)
...
```{r}
test_fixed_data_2000_final <- convert_to_numeric(test_fixed_data_2000_final)
```
...
```{r}
save_predictions_to_csv(test_2000_data_with_predictions, "EinBDW24_final_pred_c2_YuvalBakirov_EitanBakirov_ArielHedvat.csv")
```
