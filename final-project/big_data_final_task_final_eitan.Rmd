...
plot_top_10_pickup_intervals <- function(data, title, color) {
  ...
  ggplot(top_10_pickups, aes(x = time_interval, y = number_of_pickups)) +
    ...
    theme(
      ...
      plot.margin = unit(c(1, 1, 1.5, 1), "cm")          # Increase bottom margin
    ) +
...
add_is_weekend <- function(data) {
  data %>%
    mutate(
      is_weekend = ifelse(wday(time_interval) %in% c(1, 7), 1, 0)  # 1 = Sunday, 7 = Saturday
    )
}
```
...
plot_categorical_effect <- function(data, categorical_features, label, dataset_name, fill_color) {
  ...
  for (feature in categorical_features) {
    ...
    p <- ggplot(data, aes_string(x = feature, y = label, fill = feature)) +  # Use fill to color the boxes by feature
      ...
      theme_minimal() +
      theme(plot.title = element_text(size = 12, hjust = 0.5, margin = ggplot2::margin(t = 10, r = 0, b = 10, l = 0)),
...
```{r}
# Function to remove the time_interval column
remove_time_interval <- function(data) {
  # Check if the time_interval column exists in the data frame
  if ("time_interval" %in% names(data)) {
    # Remove the time_interval column
    data <- data[ , !names(data) %in% "time_interval"]
  }
  
  # Removing rows with zero Uber pickups, since these are time frames that 
  # were not given in the initial train set
...
```{r}
train_filtered_1000m_data <- remove_time_interval(train_filtered_1000m_data)
train_filtered_2000m_data <- remove_time_interval(train_filtered_2000m_data)
```

#### Fix Data Types
```{r}
# Function to convert specified columns to numeric or integer
convert_data_types <- function(data, cols_to_double = NULL, cols_to_int = NULL) {
  # Check if data is a data frame
  if (!is.data.frame(data)) {
    stop("Input data must be a data frame.")
  }
  
  # Convert specified columns to numeric (double)
...
```{r}
train_filtered_1000m_data <- convert_data_types(train_filtered_1000m_data, c("feels_like"),
                                               c("number_of_pickups", "is_weekend",
...
```{r}
test_fixed_data_1000 <- convert_data_types(test_fixed_data_1000, c("feels_like"),
                                               c("is_weekend",
                                                  "is_night", "dayInWeek", "dayInMonth",
                                                  "month", "hour", "minute",
                                                  "is_raining_last_hour", "number_of_crashes",
...
```{r}
train_filtered_2000m_data <- convert_data_types(train_filtered_2000m_data, c("feels_like"),
                                               c("number_of_pickups", "is_weekend",
                                                  "is_night", "dayInWeek", "dayInMonth",
                                                  "month", "hour", "minute",
                                                  "is_raining_last_hour", "number_of_crashes",
                                                  "taxis_pickup_count", "total_arrests",
                                                  "is_holiday", "is_persons_injured", "event_count"))
```
...
```{r}
test_fixed_data_2000 <- convert_data_types(test_fixed_data_2000, c("feels_like"),
                                               c("number_of_pickups", "is_weekend",
                                                  "is_night", "dayInWeek", "dayInMonth",
                                                  "month", "hour", "minute",
                                                  "is_raining_last_hour", "number_of_crashes",
...
Trying out the models: linear_regression, random_forest, xgboost, catboost.
* We will use k-fold cross-validation.
* Each validation_mse represents the model's average error on validation data during cross-validation for the best parameter settings, not the entire test set.
...
tune_random_forest_cv <- function(train_data, target) {
  ...
  # The whole hyperparameter grid-search
  # rf_grid <- expand.grid(
  #   mtry = c(5, 10, 15)  # Adjustable values for the number of features considered at each split
  # )
  # 
  # ntree_values <- c(100, 300, 500)  # Test different numbers of trees
...
```{r}
# tune_catboost_cv(train_data = train_filtered_1000m_data, target = "number_of_pickups")
```


### Running the models
Here is the function that runs all the models and save its results in a list:
```{r}
...
for 1000:
```{r}
# Run the models and store results in results_1000
results_1000 <- run_models(train_data = train_filtered_1000m_data, target = target)
```

Looking at the results:
```{r}
results_1000$linear_regression
```
```{r}
results_1000$random_forest
```
```{r}
results_1000$xgboost
```

```{r}
results_1000$catboost
```

#### Running 2000m
for 2000:
```{r}
# Run the models and store results in results_2000
results_2000 <- run_models(train_data = train_filtered_2000m_data, target = target)
```

Looking at the results:
```{r}
results_2000$linear_regression
```

```{r}
results_2000$random_forest
```

```{r}
results_2000$xgboost
```


```{r}
results_2000$catboost
```


```{r}
results_1000$linear_regression$feature_importance
...
```{r}
# Sort feature importance by Coefficient in descending order
sorted_importance <- results_1000$linear_regression$feature_importance %>%
...
```
```{r}
results_1000$random_forest$best_params
```

```{r}
results_1000$xgboost$best_params
```
```{r}
results_1000$catboost$best_params
```


```{r}
train_filtered_1000m_data_fs <- copy(as.data.table(train_filtered_1000m_data))
train_filtered_1000m_data_fs[, c("number_of_crashes", "is_persons_injured") := NULL]
```


```{r}
results_1000_fs <- run_models(train_data = train_filtered_1000m_data_fs, target = target)
```


```{r}
results_1000_fs$random_forest$train_mse
results_1000_fs$random_forest$validation_mse
```
```{r}
results_1000_fs$xgboost$train_mse
results_1000_fs$xgboost$validation_mse
```
```{r}

train_filtered_1000m_data_fs_cb <- copy(as.data.table(train_filtered_1000m_data))
train_filtered_1000m_data_fs_cb[, c("is_persons_injured", "is_night", "number_of_crashes") := NULL]

results_1000_fs$catboost <- tune_catboost_cv(train_filtered_1000m_data_fs_cb, "number_of_pickups")
...
```{r}
results_1000_fs$catboost$train_mse
results_1000_fs$catboost$validation_mse
```
```{r}
results_2000$linear_regression$feature_importance
results_2000$random_forest$feature_importance
results_2000$xgboost$feature_importance
results_2000$catboost$feature_importance
```


```{r}
# Sort feature importance by Coefficient in descending order
sorted_importance <- results_2000$linear_regression$feature_importance %>%
  arrange(desc(Coefficient))
...
```{r}
# Sort feature importance by Coefficient in descending order
sorted_importance <- results_2000$random_forest$feature_importance %>%
  arrange(desc(IncNodePurity))
...
```{r}
# Sort feature importance by Coefficient in descending order
sorted_importance <- results_2000$xgboost$feature_importance %>%
  arrange(desc(Gain))
...
```{r}
# Sort feature importance by Coefficient in descending order
sorted_importance <- results_2000$catboost$feature_importance %>%
  arrange(desc(Importance.V1))
...
```
```{r}
results_2000$random_forest$best_params
```

```{r}
results_2000$xgboost$best_params
```
```{r}
results_2000$catboost$best_params
```


```{r}
train_filtered_2000m_data_fs <- copy(as.data.table(train_filtered_2000m_data))
train_filtered_2000m_data_fs[, c("is_holiday", "is_persons_injured") := NULL]
```


```{r}
results_2000_fs <- run_models(train_data = train_filtered_2000m_data_fs, target = target)
```
```{r}
results_2000_fs$random_forest$train_mse
results_2000_fs$random_forest$validation_mse
```
```{r}
results_2000_fs$xgboost$train_mse
results_2000_fs$xgboost$validation_mse
```
```{r}
train_filtered_2000m_data_fs_cb <- copy(as.data.table(train_filtered_2000m_data))
train_filtered_2000m_data_fs_cb[, c("is_holiday", "is_persons_injured", "is_night", "is_weekend", "number_of_crashes") := NULL]

results_2000_fs$catboost <- tune_catboost_cv(train_filtered_2000m_data_fs_cb, "number_of_pickups")

```

```{r}
results_2000_fs$catboost$train_mse
results_2000_fs$catboost$validation_mse
```


```{r}
plot_validation_mse <- function(results_before, results_after, models, data_name) {
  
  # Extract validation MSE from the results
  validation_mse_before <- sapply(models, function(model) results_before[[model]]$validation_mse)
  validation_mse_after <- sapply(models, function(model) results_after[[model]]$validation_mse)
  
  # Combine into a data frame for plotting
  mse_data <- data.frame(
    Model = rep(models, 2),
    Validation_MSE = c(validation_mse_before, validation_mse_after),
    Feature_Selection = factor(rep(c("Before", "After"), each = length(models)), levels = c("Before", "After"))
  )
  
  # Plotting
  ggplot(mse_data, aes(x = Model, y = Validation_MSE, fill = Feature_Selection)) +
    geom_bar(stat = "identity", position = "dodge") +
    geom_text(aes(label = round(Validation_MSE, 2)), 
              position = position_dodge(width = 0.9), 
              vjust = -0.3, 
              size = 3.5) +
    labs(
      title = paste("Validation MSE Comparison Before and After Feature Selection -", data_name),
      x = "Model",
      y = "Validation Mean Squared Error (MSE)"
    ) +
    theme_minimal() +
    theme(
...
models <- c("linear_regression", "random_forest", "xgboost", "catboost")

# Call the function with your results objects for before and after feature selection
plot_validation_mse(results_before = results_1000, results_after = results_1000_fs, models = models, data_name = "1000m")
```
```{r}
# Call the function with your results objects for before and after feature selection
plot_validation_mse(results_before = results_2000, results_after = results_2000_fs, models = models, data_name = "2000m")
```


### Predicting on the Test Set
Here is the function that  uses the trained models to make predictions on a test set (we will use the model with the lowest MSE):
```{r}
# Function to predict on test data using a trained CatBoost model and fill in the target column as integers
predict_catboost_test_data <- function(test_data, target, results, model_name) {
  # Check if the specified model_name exists in results
  if (!model_name %in% names(results)) {
    stop(paste("Model", model_name, "not found in results."))
  }
  
  # Access the CatBoost model information for the specified model dynamically
  model_info <- results[[model_name]]
  
  # Prepare the data for prediction by removing the target and time_interval columns
  test_data_no_target <- test_data[, setdiff(names(test_data), c(target, "time_interval")), drop = FALSE]
  
  # Create a CatBoost Pool object for the test data
  pool_for_prediction <- catboost.load_pool(data = as.matrix(test_data_no_target))
  
  # Make predictions on the test data using the CatBoost model
  test_predictions <- catboost.predict(model_info$model, pool_for_prediction)
  
  # Round predictions down to the nearest integer
  test_predictions <- floor(test_predictions)
  
  # Fill in the target column with predictions in the test_data, converting to integer type
  test_data[[target]] <- as.integer(test_predictions)
  
  # Return the updated test data with predictions in the target column and time_interval preserved
  return(test_data)
}

```

Here is the function that saves the predictions on the test with the correct formatting:
```{r}
# Function to format test data and write to CSV
save_predictions_to_csv <- function(test_data_with_predictions, csv_name) {
  # Select only time_interval and number_of_pickups columns
  output_data <- test_data_with_predictions[, c("time_interval", "number_of_pickups")]
  
  # Format time_interval as "YYYY-MM-DDTHH:MM:SSZ"
  output_data$time_interval <- format(output_data$time_interval, "%Y-%m-%dT%H:%M:%SZ")
  
  # Write to CSV with the specified name
  write.csv(output_data, file = csv_name, row.names = FALSE)
  
  message("Data saved to ", csv_name)
}
...
As we saw, the best model we found was Catboost without the features: 
- "is_persons_injured", 
- "is_night" 
- "number_of_crashes"

which got us Average MSE Validation using CV of 130.26.

Thus, we should remove these features 
(Final 1000 data - train_filtered_1000m_data_fs_cb)
(Final 1000 model - results_1000_fs$catboost$model)


```{r}
features_to_remove <-  c("is_persons_injured", "is_night", "number_of_crashes")
test_fixed_data_1000_final <- remove_features(test_fixed_data_1000, features_to_remove)
```


```{r}
# Generate predictions and add them to test_data
test_1000_data_with_predictions <- predict_catboost_test_data(
  test_data = test_fixed_data_1000_final, 
  target = "number_of_pickups", 
  results = results_1000_fs, 
  model_name = "catboost"
)
```

```{r}
test_1000_data_with_predictions
```

```{r}
save_predictions_to_csv(test_1000_data_with_predictions, "EinBDW24_final_pred_c1_Yuval_Eitan_Ariel.csv")
```
For 2000:
...
which got us Average MSE Validation using CV of 593.65

Thus, we should remove these features 
(Final 2000 data - train_filtered_2000m_data_fs_cb)
(Final 2000 model - results_2000_fs$catboost$model)

```{r}
features_to_remove <-  c("is_holiday", "is_persons_injured", "is_night", "is_weekend", "number_of_crashes")
test_fixed_data_2000_final <- remove_features(test_fixed_data_2000, features_to_remove)
```


```{r}
# Generate predictions and add them to test_data
test_2000_data_with_predictions <- predict_catboost_test_data(
  test_data = test_fixed_data_2000_final, 
  target = "number_of_pickups", 
  results = results_2000_fs, 
  model_name = "catboost"
)
```

```{r}
test_2000_data_with_predictions
```

```{r}
save_predictions_to_csv(test_2000_data_with_predictions, "EinBDW24_final_pred_c2_Yuval_Eitan_Ariel.csv")
```
