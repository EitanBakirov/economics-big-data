...
# test_fixed_data_1000 <- remove_time_interval(test_fixed_data_1000)
# test_fixed_data_2000 <- remove_time_interval(test_fixed_data_2000)
```
...
Trying out the models: linear_regression, random_forest, xgboost.
...
Here is the function that  uses the trained models to make predictions on a test set (we will use the model with the lower MSE):
```{r}
# Function to predict on test data using a specific trained model and fill in the target column as integers
predict_test_data <- function(test_data, target, results, model_name) {
  ...
  # Access the model information for the specified model dynamically
  model_info <- results[[model_name]]
  
  # Create a copy of test_data excluding the target and time_interval columns
  test_data_no_target <- test_data[, setdiff(names(test_data), c(target, "time_interval")), drop = FALSE]
  
  # Make predictions on the test data (without the target and time_interval columns)
  test_predictions <- predict(model_info$model, newdata = test_data_no_target)
  
  # Round predictions down to the nearest integer
  test_predictions <- floor(test_predictions)
  
  # Fill in the target column with predictions in test_data, converting to integer type
...
test_1000_data_with_predictions <- predict_test_data(
  test_data = test_fixed_data_1000, 
  target = "number_of_pickups", 
...
```{r}
# Generate predictions and add them to test_data
test_2000_data_with_predictions <- predict_test_data(
  test_data = test_fixed_data_2000, 
  target = "number_of_pickups", 
