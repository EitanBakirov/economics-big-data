...
predict_test_data <- function(test_data, target, results, model_name) {
  ...
  # Create a copy of test_data excluding the target column to avoid issues with missing values
  test_data_no_target <- test_data[, setdiff(names(test_data), target), drop = FALSE]
  
  # Make predictions on the test data (without the target column)
  ...
  # Return the updated test data with predictions in the target column
  return(test_data)
}
```
